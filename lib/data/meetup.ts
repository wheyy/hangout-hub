...
export class Meetup {
    ...
    static async load(meetupId: string): Promise<Meetup | null> {
        try {
            ...
            if (!creator) {
                throw new Error("Creator user not found for meetup " + meetupId);
            }
            ...
            // Load members from user db (excluding the creator)
            if (Array.isArray(data.memberIds)) {
                for (const memberId of data.memberIds) {
                    if (memberId === meetup.creator.id) continue;
                    const member = await User.loadFromFirestore(memberId);
                    if (member) {
                        await meetup.addMember(member);
                    }
                }
            }
